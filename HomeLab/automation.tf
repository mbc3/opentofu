...
resource "proxmox_virtual_environment_container" "netboot_container" {
  description = "Netboot Container"
  ...
  initialization {
    ...
    user_account {
      password = random_password.netboot_container_password.result
      keys     = [var.ssh_key]
    }
  }
...
resource "random_password" "netboot_container_password" {
  length           = 16
  override_special = "_%@"
  special          = true
}
