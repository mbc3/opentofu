resource "proxmox_virtual_environment_container" "email_container" {
  ...
  initialization {
    ...
    user_account {
      password = random_password.email_container_password.result
      keys     = [var.ssh_key]
    }
  }
...
resource "random_password" "email_container_password" {
  length           = 16
  override_special = "_%@"
  special          = true
}
